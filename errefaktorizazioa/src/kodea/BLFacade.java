...
public class BLFacadeImplementation implements BLFacade {
	...
	public Quote createQuote(String quote, Question question, double mult) {
		dbManager.open(false);
		Quote q = null;
		q = dbManager.createQuote(new CreateQuoteParameter(quote, question), mult);
		dbManager.close();
		return q;
	}
	/**
...
*/
	...
	private boolean extracted(Vector<Question> q, boolean emaitza, Apostatu apostua) {
		for (Question quest : q) {
			int i = 0;
			boolean aurkitua = false;
	...
	private boolean extracted(boolean emaitza, Apostatu apostua, Question quest, int i, boolean aurkitua) {
		while (i < apostua.getQuote().size() && !aurkitua) {
			if (apostua.getQuote().get(i).getQuestion().equals(quest)) {
				aurkitua = true;
				System.out.println(apostua.getApostuNumber());
